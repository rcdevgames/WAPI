...
func main() {
	...
	log.Info().Msg("Open/Create "+exPath+"/dbdata/users.db")
	defer db.Close()

	sqlStmt := `CREATE TABLE IF NOT EXISTS users (id INTEGER NOT NULL PRIMARY KEY, name TEXT NOT NULL, password TEXT NOT NULL, token TEXT NOT NULL, webhook TEXT NOT NULL default "", jid TEXT NOT NULL default "", qrcode TEXT NOT NULL default "", connected INTEGER, expiration INTEGER, events TEXT NOT NULL default "All");`
	...
	if *waDebug != "" {
		...
		container, err = sqlstore.New("sqlite3", "file:./dbdata/data.db?_foreign_keys=on&_busy_timeout=3000", dbLog)
	} else {
		container, err = sqlstore.New("sqlite3", "file:./dbdata/data.db?_foreign_keys=on&_busy_timeout=3000", nil)
	}
	if err != nil {
		panic(err)
	}
